...
type Ticket struct {
	...
	UserCreatedId int       `orm:"column(user_created_id)"`
	LastUserId    int       `orm:"column(last_user_id)"`
	LastMessage   time.Time `orm:"column(last_message);type(datetime)"`
	Subject       string    `orm:"column(subject);size(255)"`
