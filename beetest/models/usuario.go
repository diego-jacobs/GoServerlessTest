...
type Usuario struct {
	...
	CodigoId            *CodigoUsuario `orm:"column(codigo_id);rel(fk)"`
	Username            string         `orm:"column(username);size(180)"`
	UsernameCanonical   string         `orm:"column(username_canonical);size(180)"`
	Email               string         `orm:"column(email);size(180)"`
	EmailCanonical      string         `orm:"column(email_canonical);size(180)"`
	...
	ConfirmationToken   string         `orm:"column(confirmation_token);size(180);null"`
	PasswordRequestedAt time.Time      `orm:"column(password_requested_at);type(datetime);null"`
	Roles               string         `orm:"column(roles)" description:"(DC2Type:array)"`
	Nombre              string         `orm:"column(nombre);size(100)"`
	Apellidos           string         `orm:"column(apellidos);size(100)"`
	ApiKey              string         `orm:"column(api_key);size(255);null"`
	Type                string         `orm:"column(type);size(255)"`
	Initials            string         `orm:"column(initials);size(255)"`
}
